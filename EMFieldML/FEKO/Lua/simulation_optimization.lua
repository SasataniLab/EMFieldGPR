--[[
    共振が取れていなかった時に最適化によって共振を取る
    コマンドライン引数として共振を取るためのパラメータを受け取っている

    Determine the electromagnetic field distribution.
    Receiving parameters as command line arguments to capture resonance.
]]

local save_file_name = io.read()
local load_pattern_file_pattern = io.read()

local C = io.read()
local voltage = io.read()
local resistance = io.read()
-- local phase = tonumber(io.read())
-- omegaL = 300
--新しいプロジェクトの作成
...
file = io.open(load_pattern_file_pattern, "r")
cnt = 1
...
-- Loadを作成
project.Variables:Add("C1", C)

properties = cf.Load.GetDefaultProperties()
properties.Capacitance = "C1"
properties.CapacitanceEnabled = true
properties.Label = "Load1"
properties.LoadType = cf.Enums.LoadTypeEnum.Series
properties.Terminal = all_port[1].Terminal
...
properties.LoadType = cf.Enums.LoadTypeEnum.Series
properties.Terminal = all_port[4].Terminal
Load2 = project.SolutionConfigurations["StandardConfiguration1"].Loads:AddLoad(properties)

properties = cf.Load.GetDefaultProperties()
properties.Resistance = resistance
properties.ResistanceEnabled = true
properties.Label = "Load3"
properties.LoadType = cf.Enums.LoadTypeEnum.Series
properties.Terminal = all_port[3].Terminal
Load3 = project.SolutionConfigurations["StandardConfiguration1"].Loads:AddLoad(properties)
...
-- -- port役割を与える
-- --complexLoad = this_config.Loads:AddComplex(port_sp.Terminal,"0.8048731382223719","-2.3099328655169447")
voltage_source1 = this_config.Sources:AddVoltageSource(all_port[2])
voltage_source1.Magnitude = voltage
voltage_source1.Phase = 0
voltage_source1.Impedance = 50

-- nearField1 = this_config.NearFields:AddCartesian(-0.4, -0.4 ,-0.2, 0.4, 0.6, 0.5, 40, 50, 45)
...
-- Created optimisation search: "Search1"
properties = cf.OptimisationSearch.GetDefaultProperties()
properties.Label = "Search1"
Search1 = project.Optimisation.Searches:Add(properties)

-- Created optimisation operator: "ImpedanceGoal1"
...
VoltageSource1 = voltage_source1
properties.FocusSource = VoltageSource1
properties.FocusSourceLabel = "VoltageSource1"
properties.GoalOperator = cf.Enums.OptimisationGoalOperatorEnum.LessThan
properties.Label = "ImpedanceGoal1"
properties.Objective.TargetValue = "5.0"
properties.ProcessingSteps[1].Operation = cf.Enums.OptimisationGoalProcessingStepsEnum.Phase
properties.ProcessingSteps[2] = {}
properties.ProcessingSteps[2].Operation = cf.Enums.OptimisationGoalProcessingStepsEnum.AbsoluteValue
ImpedanceGoal1 = Search1.Goals:AddImpedanceGoal(properties)

OptimisationParameters = project.Optimisation.Searches["Search1"].Parameters
properties = OptimisationParameters:GetProperties()
properties.Variables[1] = {}
properties.Variables[1].Enabled = true
properties.Variables[1].MaximumValue = C * 1.0
properties.Variables[1].MinimumValue = C * 0.995
properties.Variables[1].NumberOfGridPoints = ""
properties.Variables[1].StartValue = C * 0.9975
C1 = project.Variables["C1"]
properties.Variables[1].Variable = C1
OptimisationParameters:SetProperties(properties)
